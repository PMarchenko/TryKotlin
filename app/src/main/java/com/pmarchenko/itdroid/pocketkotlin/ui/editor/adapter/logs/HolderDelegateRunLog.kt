 ...
 */
class HolderDelegateRunLog(private val callback: EditorCallback) : HolderDelegateLog<RunLogRecord, HolderDelegateRunLog.RunLogViewHolder>() {
    ...
    class RunLogViewHolder(itemView: View, callback: EditorCallback) : LogViewHolder<RunLogRecord>(itemView, callback) {
        ...
        override fun prepareText(log: RunLogRecord): CharSequence {
            ...
            if (log.args.isEmpty()) {
                ...
                text.append(resources.getString(R.string.logs__template__run_with_args, log.projectName, log.args))
            }
            return text
        }
    }
